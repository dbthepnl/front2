   ...
   <div class="space-y-4">
      <form @submit.prevent="postComment" class="w-full mt-4">
        ...
        <p class="text-right mb-6">
          <button type="submit" class="ml-3 px-3 py-2 text-md text-blue-100 bg-blue-600 rounded inline-flex items-center">답변하기 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="pl-2 w-6 h-6">
  ...
  <div class="space-y-4 mt-4">
    <div v-for="row in comment" :key="row" class="flex">
      <div class="flex-1 border rounded-lg p-4 md:p-6 lx:p-6 leading-relaxed">
        ...
        <p><button @click="deleteComment(row.id)" class="mt-10 inline-block px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg transition duration-150 ease-in-out">삭제하기</button></p>
      </div>
    </div>
   <!-- Replay
    <div class="flex">
